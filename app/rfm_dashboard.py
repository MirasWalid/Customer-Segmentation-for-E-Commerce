...
with tab1:
    ...
    try:
        with open("../data/classification_report.json", "r", encoding="utf-8") as f:
            metrics = json.load(f)

        # Accuracy is stored at top level
        accuracy = float(metrics.get("accuracy", 0))

        # Precision/Recall/F1 are inside "macro avg" or "weighted avg"
        if "macro avg" in metrics:
            agg = metrics["macro avg"]
        else:
            agg = metrics.get("weighted avg", {})

        precision = float(agg.get("precision", 0))
        recall    = float(agg.get("recall", 0))
        f1        = float(agg.get("f1-score", 0))
        ...
        accuracy = precision = recall = f1 = 0
        st.error(f"Error reading classification_report.json: {e}")
